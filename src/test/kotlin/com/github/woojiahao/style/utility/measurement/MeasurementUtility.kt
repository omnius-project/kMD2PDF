...
val allInputs: List<List<Number>> = listOf(
  potentialByteInputs,
...
fun assertMeasurementIndexed(type: Measurement.Type, assert: (Pair<Int, Int>, Number, Measurement<*>?) -> Unit) {
  allInputs.forEachIndexed { outerIndex, list ->
    list.forEachIndexed { innerIndex, num ->
      val measurement = when (num) {
        is Int -> num match type
        is Long -> num match type
        is Double -> num match type
      ...
      assert(outerIndex to innerIndex, num, measurement)
    }
  }
}